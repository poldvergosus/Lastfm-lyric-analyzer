...
import { startAnalysis, getStatus } from "../api/client";
...
export function useAnalysis() {
  ...
  const run = useCallback(
    async (username: string, from: string, to: string, maxTracks: number) => {
      setResult(null);
      stop();
      ...
      try {
        const taskId = await startAnalysis(username, from, to, maxTracks);
